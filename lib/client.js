	...
	factory: (require) => {
		...
		*/
		...
		var WebChatApi = class {
			...
			transfer(chatId, cwd, mode, workspaceId) {
		...
		function WebChatPanel({ api, tt, sessions, workspaces, currentCwd }) {
			...
			const transferToHarness = (0, react.useCallback)(async () => {
				...
				try {
					...
					if (result.attached === false && targetWorkspaceId !== void 0) showToast(fmt(tt("transfer.attached.failed"), { sessionId: result.sessionId }), true);
					else showToast(fmt(tt("transfer.done"), { sessionId: result.sessionId }));
