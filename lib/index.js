...
function blankOf(values) {
	return (values?.["sessionListMetadata"])?.blank === true;
}
/** List every harness session (live + cold) with a readable title, newest first. */
async function listHarnessSessions(ctx) {
	...
	const persistence = ctx.get("sessionPersistence");
	if (persistence !== void 0) for (const meta of await persistence.list()) {
		if (seen.has(meta.id) || meta.cwd === void 0) continue;
		...
		},
		{
			...
			handler: async (req, res) => {
				...
				try {
					writeJson(res, 200, {
						ok: true,
						sessions: await listHarnessSessions(ctx)
					});
				} catch (error) {
					writeJson(res, 500, {
						ok: false,
