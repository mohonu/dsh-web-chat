	...
	const mcpRoutes = mcpBridge ? [{
		...
		handler: async (req, res) => {
			...
			try {
				...
				writeJson(res, 200, {
					ok: true,
					result: await mcpBridge.callTool({
						tool: body.tool,
						arguments: body.arguments && typeof body.arguments === "object" && !Array.isArray(body.arguments) ? body.arguments : {}
					})
				});
			} catch (error) {
				writeJson(res, 500, {
...
function webChatStatusTool(engine, store, listWorkspaces, mcpBridge) {
	return defineTool({
		name: "webchat_status",
		...
		async execute() {
			...
			if (mcpBridge !== void 0) {
				...
				if (mcpTools.length > 0) lines.push(`mcpTools: ${mcpTools.map((t) => t.name).join(", ")}`);
				else lines.push("mcpTools: (none)");
			}
...
function webChatSendTool(engine, mcp) {
	return defineTool({
		name: "webchat_send",
		...
		async execute(args) {
			...
			const result = await (mcp ?? engine).send(textValue, true, images);
			return {
				reply: result.reply ?? "",
				error: result.error,
				code: result.code,
...
var WebChatMcpBridge = class {
	...
	listTools() {
		...
		if (typeof tools.list !== "function") return [];
		const result = tools.list();
		if (!Array.isArray(result)) return [];
		return result.filter((tool) => {
			return typeof tool === "object" && tool !== null && typeof tool.name === "string" && tool.name.startsWith("mcp__");
		}).map(normalizeTool);
	}
	async callTool(request) {
		if (!request.tool.startsWith("mcp__")) throw new Error(`Refusing non-MCP tool: ${request.tool}`);
		const tools = getRegistry(this.ctx);
		if (typeof tools.get === "function") {
			const tool = tools.get(request.tool);
			if (!tool) throw new Error(`MCP tool not found: ${request.tool}`);
			if (typeof tool.execute !== "function") throw new Error(`MCP tool is not executable: ${request.tool}`);
			return normalizeResult(await tool.execute(request.arguments ?? {}));
		}
		if (typeof tools.execute === "function") return normalizeResult(await tools.execute(request.tool, request.arguments ?? {}));
...
const MCP_CALL_RE = /<dsh_mcp_call>\s*([\s\S]*?)\s*<\/dsh_mcp_call>/i;
function extractMcpCall(markdown) {
	...
	try {
		...
		throw new Error("DeepSeek produced invalid MCP call JSON");
	}
	if (typeof parsed !== "object" || parsed === null) throw new Error("DeepSeek MCP call must be an object");
	const value = parsed;
	if (typeof value.tool !== "string" || !value.tool.startsWith("mcp__")) throw new Error("Invalid MCP tool name");
	const args = value.arguments;
	if (args !== void 0 && (typeof args !== "object" || args === null || Array.isArray(args))) throw new Error("MCP arguments must be an object");
	return {
		tool: value.tool,
		arguments: args ?? {}
	};
}
function buildMcpPrompt(tools) {
...
function buildMcpResultMessage(call, result) {
	return [
		"MCP TOOL RESULT",
		"",
		`Tool: ${call.tool}`,
...
function buildMcpErrorMessage(call, error) {
	return [
		"MCP TOOL ERROR",
		"",
		`Tool: ${call.tool}`,
...
//#endregion
//#region src/mcp-orchestrator.ts
var McpOrchestrator = class {
	engine;
	bridge;
	maxRounds;
	enabled;
	constructor(engine, bridge, maxRounds = 8, enabled = true) {
		this.engine = engine;
		this.bridge = bridge;
		this.maxRounds = maxRounds;
		this.enabled = enabled;
	}
	async send(message, wait, images) {
		if (!this.enabled) return this.engine.send(message, wait, images);
		const mcpPrompt = buildMcpPrompt(this.bridge.listTools());
		let currentMessage = mcpPrompt ? `${mcpPrompt}
...
${message}` : message;
		...
		for (let round = 0; round < this.maxRounds; round++) {
			const result = await this.engine.send(currentMessage, wait, currentImages);
			lastResult = result;
			console.log("[MCP DEBUG] raw reply:", result.reply);
			if (!result.ok || !result.reply) break;
			let mcpCall;
			try {
...
const Config = import_lib.default.object({
	...
	mcpEnabled: import_lib.default.boolean().default(true)
});
...
const WEBCHAT_GUIDANCE = "本机已安装 dsh-webchat 插件（Codex ChatGPT 模式 · DeepSeek 网页端聊天）：侧边栏「网页聊天」入口；通过真实浏览器驱动 chat.deepseek.com（DeepSeek 网页模型，深度思考/智能搜索开关，网页登录会话，无需 API 额度）。能力：webchat_status 查看登录/会话状态、webchat_send 通过网页端发送消息并流式获取回复（可附带本地图片路径做多模态提问；当 DSH 中注册了 MCP 工具时，网页端模型可自动调用这些工具获取实时数据）、webchat_recover 把网页端已有会话同步/恢复到本地、webchat_import 把存储的网页对话导入为 markdown 上下文、webchat_transfer 把网页对话蒸馏成可执行任务简报并创建新 harness 会话（首条消息即任务简报，而非原始聊天记录），或经 targetSessionId 把简报作为新消息追加到已有会话延续同一任务；GUI 面板可将对话随时导出为工作区文件或转入 harness 会话。限制：首次使用需用户在弹出的浏览器窗口完成 DeepSeek 网页登录；网页端受 DeepSeek 官方风控，操作失败或页面改版时返回错误而非崩溃；面板提供「深度思考（R1）」与「智能搜索」开关（网页端无模型选择器）。用户提到「网页聊天 / 网页端 / ChatGPT 模式 / deepseek web / 转移到 harness」时即指本插件，请据此协作。";
